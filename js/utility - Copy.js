    ...
    container.innerHTML = `
        ...
        <div class="form-box">
            <div class="input-group">
                ...
                <select id="doc-type-selector" onchange="handleDocSelection()" style="border: 2px solid #8B5CF6; font-size: 16px; padding: 12px; min-height: 55px; line-height: 1.5; font-family: 'Prompt', sans-serif; width: 100%; border-radius: 8px;">
...
window.updateYp6Plate = function() {
    ...
    filteredCars.forEach(c => {
        // 🔴 นำยี่ห้อรถออก เหลือเฉพาะทะเบียนตามคำขอ
...
function openYP6Modal() {
    const now = new Date();
    const firstDay = `01/${String(now.getMonth() + 1).padStart(2, '0')}/${now.getFullYear() + 543}`;
    ...
    Swal.fire({
        ...
        html: `
            <div style="text-align: left; font-family: 'Prompt', sans-serif;">
                <div class="flex-row" style="gap: 10px;">
                    <div class="input-group flex-col"><label>ตั้งแต่วันที่</label><input type="text" id="yp6-start" class="swal2-input" style="margin:0; width:100%; padding:12px; min-height:50px; border-radius:6px;" value="${firstDay}"></div>
                    <div class="input-group flex-col"><label>ถึงวันที่</label><input type="text" id="yp6-end" class="swal2-input" style="margin:0; width:100%; padding:12px; min-height:50px; border-radius:6px;" value="${today}"></div>
                </div>
...
// 🔴 ระบบแกะวันที่อัจฉริยะ (ดึงวันที่จากข้อความทุกแพทเทิร์น)
function extractDate(str) {
    if (!str) return null;
    let s = String(str).trim();
    
    // ดักจับรูปแบบ DD/MM/YYYY
    let matchSlash = s.match(/(\d{1,2})[\/](\d{1,2})[\/](\d{4})/);
    if (matchSlash) {
        let d = parseInt(matchSlash[1]); let m = parseInt(matchSlash[2]); let y = parseInt(matchSlash[3]);
        if (y > 2500) y -= 543;
        return new Date(y, m - 1, d);
    }
    
    // ดักจับรูปแบบ YYYY-MM-DD
...
async function fetchAndGenerateYP6(params) {
    ...
    try {
        // 🔴 ปรับจูนการรับข้อมูล Array จาก API เพื่อป้องกัน Error "undefined บรรทัด" ให้จับ Object ได้ครบทุกมิติ
        let res;
        try {
            res = await apiCall({ action: 'get_car_logs' }); 
            // ถ้าระบบไม่ได้ใช้ action 'get_car_logs' ลองดึงจาก 'get_reports' แทน
        ...
        if(allLogs.length === 0) {
            throw new Error("ระบบหลังบ้าน (Google Sheets) ไม่ส่งข้อมูลกลับมา");
        }
        
        // ช่วงวันที่ค้นหา
        const [sDay, sMonth, sYear] = params.startDate.split('/');
        const startD = new Date(parseInt(sYear) - 543, parseInt(sMonth) - 1, parseInt(sDay));
        startD.setHours(0,0,0,0);
        
        const [eDay, eMonth, eYear] = params.endDate.split('/');
        const endD = new Date(parseInt(eYear) - 543, parseInt(eMonth) - 1, parseInt(eDay));
        endD.setHours(23,59,59,999);

        // 🔴 ระบบกรองข้อมูล (Filter) ให้แม่นยำ 100% ตามหัวคอลัมน์ภาษาไทย
        let historyLogs = allLogs.filter(log => {
            ...
            // 🔴 ดึงวันที่เจาะจงจากหัวคอลัมน์ "วัน" หรือ "วันที่เวลารับ" ตามคำสั่ง
            let logDateStr = String(log['วัน'] || log['วันที่เวลารับ'] || log['วันที่'] || log.timestamp || log.createDate || ''); 
            let logD = extractDate(logDateStr);
        ...
        if (historyLogs.length === 0) {
            Swal.fire({
                title: 'ไม่พบประวัติ',
                html: `ไม่พบบันทึกข้อมูลของรถทะเบียน <b>${params.plate}</b><br>ในช่วงวันที่กำหนด<br><br><small style="color:var(--text-light);">(ข้อมูล CarLogs ทั้งหมดมี ${allLogs.length} รายการ)</small>`,
                icon: 'info'
            });
            return;
        }
        ...
        // วนลูปวาดตารางและดึงข้อมูลจาก CarLogs (รองรับคีย์ภาษาไทย)
        historyLogs.forEach(log => {
            let logDateStr = String(log['วัน'] || log['วันที่เวลารับ'] || log['วันที่'] || log.timestamp || ''); 
            let matchDate = logDateStr.match(/(\d{1,2})[\/|-](\d{1,2})[\/|-](\d{4})/);
            let displayDate = matchDate ? `${matchDate[1]}/${matchDate[2]}/${matchDate[3]}` : '-';
            
            let empName = log['ผู้ปฏิบัติงาน'] || log['ชื่อผู้ขอ'] || log.empName || log.name || '-';
            let location = log['สถานที่ปฏิบัติงาน'] || log['ไปปฏิบัติงาน'] || log.location || log['สถานที่'] || '-';
            let startMile = log['เลขไมล์ก่อนใช้'] || log['ขาไป'] || log.startMile || '-';
            let endMile = log['เลขไมล์หลังใช้'] || log['ขากลับ'] || log.endMile || '-';
            let fuel = log['น้ำมันเชื้อเพลิงที่เติมแต่ละครั้ง'] || log['น้ำมัน'] || log.fuel || '';
        ...
        <head>
            ...
            <style>
                ...
                html, body { font-family: 'Sarabun', 'TH Sarabun New', sans-serif !important; font-size: 13pt !important; color: #000 !important; background: #fff; margin: 0; padding: 0; -webkit-print-color-adjust: exact; }
                .header-title { font-size: 16pt; font-weight: bold; text-align: center; margin-bottom: 10px; }
                .sub-title { font-size: 14pt; margin-bottom: 10px; display: flex; justify-content: space-between; }
                table { width: 100%; border-collapse: collapse; border: 1px solid #000; }
                th { border: 1px solid #000; text-align: center; padding: 8px 4px; font-weight: bold; vertical-align: middle; line-height: 1.4 !important; }
                td { border-left: 1px solid #000; border-right: 1px solid #000; border-bottom: 1px dotted #000; padding: 4px; vertical-align: top; height: 35px; }
        ...
        <body>
            ...
            <div class="sub-title">
                ...
                <div><b>ช่วงวันที่:</b> ${params.startDate} ถึง ${params.endDate}</div>
            </div>
            <table>
                <thead>
                    <tr>
